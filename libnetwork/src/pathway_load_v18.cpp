...
/*
  ...
  is there, but without a means to tell the QDomClasses what Ids are
  the method always returns the null element. - v3.2.1
*/

static bool parseEditor(QDomNode &editor_node, Editor &editor);
static bool parseSimulation(QDomNode &simulation_node, Simulation &simulation, Fitness &fitness);
...
parseEditor(QDomNode &editor_node, Editor &editor) {
  ...
  while(!n.isNull()) {
	...
	editor.use_grid_ = true;
      } else if(e.tagName() == "DisplayGrid") {
	editor.display_grid_ = true;
      } else if(e.tagName() == "DrawIcon") {
      } else if(e.tagName() == "DrawLabel") {
...
static bool
parseSimulation(QDomNode &simulation_node, Simulation &simulation, Fitness &fitness) {
  ...
  while(!n.isNull()) {
	...
	simulation.setHistogramUpdateRate(e.text().toInt());
      } else if(e.tagName() == "Environment") {
	QString str = e.text().simplifyWhiteSpace();
	simulation.setNumEnvironment(str.section(' ', 1, 1).toInt());
	simulation.setCurEnvironment(str.section(' ', 0, 0).toInt()-1);
      } else if(e.tagName() == "FitnessFunction") {
...
bool
pathwayLoad_v18(QDomDocument &doc, Editor &editor, Fitness &fitness, Simulation &simulation,
  ...
  while(!n.isNull()) {
	...
	if(!parseSimulation(n, simulation, fitness)) return false;
