...
main(int argc, char *argv[]) {
  ...
  for(i=1; i<argc; i++) {
    ...
    ppm_w = border+(color_bar_width == 0 ? 0 : color_bar_width+border)+w+border;
    ppm_h = border+h+border;
    fprintf(file, "P6\n%d %d\n%d\n", ppm_w, ppm_h, 255);

    ppm = malloc(ppm_w*ppm_h*3);
    memset(ppm, 0, ppm_w*ppm_h*3);

    if(color_bar_width > 0) {
      for(y=0; y<h; y++) {
	z = y/(double)h;
	HSVtoRGB(z*240, 1.0, 1.0, color);
	for(x=0; x<color_bar_width; x++) {
	  int a = ((y+border)*ppm_w+(x+border))*3;
	  ppm[a  ] = (int)(color[0] * 255 + 0.5);
	  ppm[a+1] = (int)(color[1] * 255 + 0.5);
	  ppm[a+2] = (int)(color[2] * 255 + 0.5);
	}
      }
    }
    ...
    for(y=0; y<h; y++) {
      for(x=0; x<w; x++) {
	int a = ((y+border)*ppm_w+(x+border+(color_bar_width == 0 ? 0 : color_bar_width+border)))*3;
	z = matrix[y*w+x];
	...
	x = (int)(kc2 / step[1] + 0.5);
        ...
        for(j=-5; j<=5; j++) {
	  int a = ((y+border+j)*ppm_w+(x+border+(color_bar_width == 0 ? 0 : color_bar_width+border)))*3;
	  int b = ((y+border)*ppm_w+(x+border+(color_bar_width == 0 ? 0 : color_bar_width+border)+j))*3;
  	  ppm[a  ] = 0;
	  ppm[a+1] = 0;
	  ppm[a+2] = 0;
