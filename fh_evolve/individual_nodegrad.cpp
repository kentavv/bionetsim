...
#else
      double ptp[5] = { 0.50, // phosphorylation
			1.00, // complex formation
			1.10, // catalytic degradation
			1.10, // partial protein complex degradation (complex required)
			1.10  // partial protein complex catalytic degradation (complex required)
#endif
      };
...
Individual::fitness() {
  if(!current_ || final) {
    ...
    f_ = sqrt( ((tid & 0x01) ? sqr(res[3] - 100) : sqr(res[3] - 1)) +
	       ((tid & 0x02) ? sqr(res[2] - 100) : sqr(res[2] - 1)) +
	       ((tid & 0x04) ? sqr(res[1] - 100) : sqr(res[1] - 1)) +
	       ((tid & 0x08) ? sqr(res[0] - 100) : sqr(res[0] - 1)) );
