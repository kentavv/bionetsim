...
Individual::Individual()
  : current_(false),
    gene_ind_(0)
{
  // Initialize reactions to contain two genes and associated protein productions and degradations
  addGene();
  addGene();  
}
...
Individual::mutate() {
  for(int mut=0; mut<nmut; mut++) {
    ...
    double p[5] = { 0.30, 0.60, 0.65, 0.75, 1.0 };
...
Individual::fitness() {
  if(!current_ || final) {
    ...
    //if(!pathwayLoad("s4.net", items)) abort();
    ...
    if(!p1 || !p2) {
      fprintf(stderr, "Could not find p1 or p2\n");
      exit(EXIT_FAILURE);
    }
    
    Result r;
    double res[4][2] = { {0, 0}, {0, 0}, {0, 0}, {0, 0} };

    // Run to reach stable values
    {
    ...
    // Pulse P1 to get to assumed initial state of P1 high
    {
      applyIntialConcentrations(r, items);
      p1->setInitialConcentration(p1->initialConcentration()*100);
      pm.setup(editor, pm.simulation, items);
    ...
    // Record last stable point, pulse P2
    {
      applyIntialConcentrations(r, items);
      res[0][0] = p1->initialConcentration();
      res[0][1] = p2->initialConcentration();      
      p2->setInitialConcentration(p2->initialConcentration()*100);
      pm.setup(editor, pm.simulation, items);
    ...
    {
      applyIntialConcentrations(r, items);
      res[1][0] = p1->initialConcentration();
      res[1][1] = p2->initialConcentration();      
      p1->setInitialConcentration(p1->initialConcentration()*100);
      pm.setup(editor, pm.simulation, items);
    ...
    {
      applyIntialConcentrations(r, items);
      res[2][0] = p1->initialConcentration();
      res[2][1] = p2->initialConcentration();      
      p2->setInitialConcentration(p2->initialConcentration()*100);
      pm.setup(editor, pm.simulation, items);
    ...
    {
      applyIntialConcentrations(r, items);
      res[3][0] = p1->initialConcentration();
      res[3][1] = p2->initialConcentration();      
    }
    
#if 0
    cv1->setValues(1, 0);
    cv2->setValues(0, 0);
    if(!run_simulation(pm, r[2])) {
      return f_;
    }

    cv1->setValues(1, 0);
    cv2->setValues(1, 0);
    if(!run_simulation(pm, r[3])) {
      return f_;
    }
#endif
    ...
    int nr3 = r[3].m.nrows();
    //printf("%d %d %d %d\n", nr0, nr1, nr2, nr3);
    f_ += sqr(r[0].m(nr0-1, 1+1+nc) - 10.0);
    f_ += sqr(r[0].m(nr0-1, 3+1+nc) - 0.0);

    f_ += sqr(r[1].m(nr1-1, 1+1+nc) - 0.0);
    f_ += sqr(r[1].m(nr1-1, 3+1+nc) - 10.0);

    f_ += sqr(r[2].m(nr2-1, 1+1+nc) - 10.0);
    f_ += sqr(r[2].m(nr2-1, 3+1+nc) - 0.0);

    f_ += sqr(r[3].m(nr3-1, 1+1+nc) - 0.0);
    f_ += sqr(r[3].m(nr3-1, 3+1+nc) - 10.0);
#endif
    f_ += sqr(res[0][0] - 10.0);
    f_ += sqr(res[0][1] -  0.0);
    ...
    int i;
    for(i=0; i<1000*10; i++) {
      f_ += (r.m(i, 1+1+nc) - 10.0) * (r.m(i, 1+1+nc) - 10.0);
      f_ += (r.m(i, 3+1+nc) -  0.0) * (r.m(i, 3+1+nc) -  0.0);
    }
    for(; i<2000*10; i++) {
      f_ += (r.m(i, 1+1+nc) -  0.0) * (r.m(i, 1+1+nc) -  0.0);
      f_ += (r.m(i, 3+1+nc) - 10.0) * (r.m(i, 3+1+nc) - 10.0);
    }
    for(; i<3000*10; i++) {
      f_ += (r.m(i, 1+1+nc) - 10.0) * (r.m(i, 1+1+nc) - 10.0);
      f_ += (r.m(i, 3+1+nc) -  0.0) * (r.m(i, 3+1+nc) -  0.0);
...
#if 0
    t_ = (((r[0].m(nr0-1, 1+1+nc) - r[1].m(nr1-1, 1+1+nc) > 1.0 &&
	    r[1].m(nr1-1, 1+1+nc) - r[2].m(nr2-1, 1+1+nc) < 1.0 &&
	    r[2].m(nr2-1, 1+1+nc) - r[3].m(nr3-1, 1+1+nc) > 1.0) ? 1 : 0) +
	  ((r[0].m(nr0-1, 3+1+nc) - r[1].m(nr1-1, 3+1+nc) < 1.0 &&
	    r[1].m(nr1-1, 3+1+nc) - r[2].m(nr2-1, 3+1+nc) > 1.0 &&
	    r[2].m(nr2-1, 3+1+nc) - r[3].m(nr3-1, 3+1+nc) < 1.0) ? 2 : 0));
#endif

    t_ = (((res[0][0] - res[1][0] >  1.0 &&
	    res[1][0] - res[2][0] < -1.0 &&
	    res[2][0] - res[3][0] >  1.0) ? 1 : 0) +
	  ((res[0][1] - res[1][1] < -1.0 &&
	    res[1][1] - res[2][1] >  1.0 &&
	    res[2][1] - res[3][1] < -1.0) ? 2 : 0));
...
Individual::save2(const char *filename) {
  {
    ...
    QString text;
    buildMoleculeReducedEquations(items, text, 0);
...

  {
    ...
    Editor editor;
    Fitness fitness;
    return pathwaySave(filename, editor, fitness, pm.simulation, items);
  }
}

bool
Individual::convertToItems(QList<PathwayCanvasItem> &items, bool includeCVs) {
  items.clear();
